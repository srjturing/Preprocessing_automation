...
# ─────────────────────────────────────────────────────────────────────────────
# HTTP CLIENT (Session with retries)
# ─────────────────────────────────────────────────────────────────────────────
def make_session(verify_ssl: bool = True) -> requests.Session:
    ...
    s.verify = verify_ssl
    s.headers.update({"User-Agent": "turing-preprocess-uploader/1.1"})
    return s
...
def _format_http_error(resp: requests.Response) -> str:
    ...
    return (f"HTTP {resp.status_code}\n"
            f"Request: {resp.request.method} {resp.url}\n"
            f"Resp-Headers: {dict(resp.headers)}\n"
            f"Body: {body[:2000]}")
...
def upload_csv_bytes(session: requests.Session, csv_bytes: bytes, filename: str, token: str,
    ...
    data    = {"project_type": "rlhf"}  # switch to "pdf" for PDF projects
    ...
    if not link:
        raise Exception(f"Upload step missing 'fileLink' field:\n{payload}")
...
def upload_pipeline(csv_bytes: bytes, filename: str, *, token: str, project_id: int, project_name: str,
    ...
    try:
        ...
        return True, f"Uploaded & imported • Batch ID: {batch_id}"
...
def show_levels_example_dialog(levels_list):
    ...
    try:
        with st.dialog("Example: understanding `level_*` columns"):
            ...
            st.code("\n".join(lines)); st.caption(note)
            if st.button("Got it", type="primary", key="levels_got_it"):
                st.session_state.show_levels_modal = False; st.rerun()
    except Exception:
    ...
    for wi in data.get("workitems", []):
        ...
        for k, v in wi.items():
            ...
            if isinstance(v, dict):
                if k == "inputData":
                    ...
                    for ik, iv in v.items():
                        if re.fullmatch(r"Image_\d+", ik):
                            ...
                            row.update(_flatten_leaves(iv, f"workitems-{k}"))
                        else:
                ...
                row[f"workitems-{k}"] = v
        rows.append({**file_meta_flat, **row})
...
with st.sidebar:
    ...
    verify_ssl = st.toggle(
        "Verify SSL certificates", value=True,
        help="Turn OFF only for debugging corporate proxy/TLS issues."
    )
    connect_timeout = st.number_input("Connect timeout (sec)", min_value=5, max_value=120,
                                      value=DEFAULT_CONNECT_TIMEOUT, step=1)
    read_timeout    = st.number_input("Read timeout (sec)", min_value=60, max_value=1800,
                                      value=DEFAULT_READ_TIMEOUT, step=30)
...
# ─────────────────────────────────────────────────────────────────────────────
# MODES: Load base_df + available_fields
# ─────────────────────────────────────────────────────────────────────────────
if mode == "CSV/Excel":
    ...
    try:
        ...
        st.success(f"Loaded {len(base_df)} rows.")
...
# ─────────────────────────────────────────────────────────────────────────────
# OUTPUT: STEP 2 — FIELD SELECTIONS
# ─────────────────────────────────────────────────────────────────────────────
...
if submitted:
    ...
    st.success(f"✅ Generated {len(out_df)} rows • {out_df.shape[1]} columns.")
...
# ─────────────────────────────────────────────────────────────────────────────
# SPLIT, DOWNLOAD & UPLOAD (REWRITTEN: no big objects in session)
# ─────────────────────────────────────────────────────────────────────────────
if "out_df" in st.session_state:
    ...
    if default_split not in st.session_state.get("split_field", "None"):
        default_split = "None"
    ...
    # SINGLE CSV PATH
    if str(split_field).strip().lower() == "none":
        # cache single csv in memory (string only; not kept across actions unnecessarily)
        buf = io.StringIO(); out_df.to_csv(buf, index=False)
        csv_text = buf.getvalue()
        c_dl, c_up = st.columns([0.5, 0.5])
        with c_dl:
            st.download_button("⬇️ Download CSV", data=csv_text, file_name="output.csv",
                               mime="text/csv", key="single_csv_dl", use_container_width=True)
        with c_up:
            with st.expander("📤 Upload this CSV to the tool"):
                ...
                if st.button("🚀 Upload output.csv", use_container_width=True, key="single_go"):
                    if not (token and project_id and project_name):
                        ...
                        ok, msg = upload_pipeline(
                            csv_bytes=csv_text.encode("utf-8"), filename="output.csv",
                            token=token, project_id=int(project_id), project_name=project_name,
    ...
    # Build groups lightweightly
    groups: Dict[str, List[int]] = {}
    for idx, val in series.items():
        key_val = _safe_name(val if pd.notna(val) else "UNSPECIFIED")
        groups.setdefault(key_val, []).append(idx)

    # Selection (store only names, not CSVs)
    if f"selected_groups__{split_field}" not in st.session_state:
        st.session_state[f"selected_groups__{split_field}"] = []
    selected_groups = st.multiselect(
        "Select groups to act on (for Selected actions below):",
        options=sorted(groups.keys()),
        default=st.session_state[f"selected_groups__{split_field}"],
        key=f"msel_{split_field}"
    )
    st.session_state[f"selected_groups__{split_field}"] = selected_groups

    # Quick preview table
    st.caption("Group sizes:")
    preview = pd.DataFrame(
        [{"group": g, "rows": len(idxs)} for g, idxs in sorted(groups.items())]
    )
    ...
    with cA:
        ...
        # All groups as ZIP
        zip_all = io.BytesIO()
        with zipfile.ZipFile(zip_all, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
            for g, idxs in groups.items():
                zf.writestr(f"output_{_safe_name(split_field)}_{g}.csv",
                            out_df.loc[idxs].to_csv(index=False))
        zip_all.seek(0)
        st.download_button(
            f"Download ALL ({len(groups)})",
            data=zip_all.getvalue(),
            file_name=f"output_by_{_safe_name(split_field)}.zip",
            mime="application/zip",
            key=f"zip_all_{split_field}",
            use_container_width=True,
        )
        # Selected as ZIP
        if selected_groups:
            zip_sel = io.BytesIO()
            with zipfile.ZipFile(zip_sel, mode="w", compression=zipfile.ZIP_DEFLATED) as zf:
                for g in selected_groups:
                    zf.writestr(f"output_{_safe_name(split_field)}_{g}.csv",
                                out_df.loc[groups[g]].to_csv(index=False))
            zip_sel.seek(0)
            st.download_button(
                f"Download Selected ({len(selected_groups)})",
                data=zip_sel.getvalue(),
                file_name=f"output_selected_by_{_safe_name(split_field)}.zip",
                mime="application/zip",
                key=f"zip_sel_{split_field}",
                use_container_width=True,
            )
        else:
            st.button("Download Selected (choose groups above)", disabled=True, key=f"zip_sel_disabled_{split_field}")

    # 2) MERGED SELECTED
    with cB:
        st.markdown("**🧩 Merge Selected → CSV**")
        if selected_groups:
            merged_df = pd.concat([out_df.loc[groups[g]] for g in selected_groups], ignore_index=True)
            ...
            st.download_button(
                ...
                mime="text/csv",
                key=f"dl_merged_{split_field}",
                use_container_width=True,
            )

            with st.expander("📤 Upload Merged to Tool"):
                token_m        = st.text_input("TOKEN", type="password", key=f"tok_merge_{split_field}")
                project_id_m   = st.text_input("PROJECT_ID", key=f"pid_merge_{split_field}")
                project_name_m = st.text_input("PROJECT_NAME", key=f"pname_merge_{split_field}")
                if st.button("🚀 Upload Merged CSV", key=f"btn_merge_ul_{split_field}", use_container_width=True):
                    if not (token_m and project_id_m and project_name_m):
                        st.error("Please fill TOKEN, PROJECT_ID and PROJECT_NAME.")
                    else:
                        ok, msg = upload_pipeline(
                            csv_bytes=merged_csv_str.encode("utf-8"),
            ...
            st.info("Select at least one group to build a merged CSV.")

    # 3) INDIVIDUAL UPLOADS (SELECTED or ALL) — stateless per run
    with cC:
        st.markdown("**🚀 Upload Individually**")
        token_up        = st.text_input("TOKEN", type="password", key=f"tok_ind_{split_field}")
        project_id_up   = st.text_input("PROJECT_ID", key=f"pid_ind_{split_field}")
        project_name_up = st.text_input("PROJECT_NAME", key=f"pname_ind_{split_field}")
        delay_secs      = st.number_input("Delay (sec) between uploads", min_value=0, max_value=300, value=0, step=1,
                                          key=f"delay_ind_{split_field}")
        target_mode     = st.radio("Which files?", options=["Selected", "All"], horizontal=True, key=f"mode_ind_{split_field}")

        if st.button("Start Upload", key=f"go_ind_{split_field}", use_container_width=True):
            if not (token_up and project_id_up and project_name_up):
                st.error("Please fill TOKEN, PROJECT_ID and PROJECT_NAME.")
            else:
                targets = selected_groups if target_mode == "Selected" else list(groups.keys())
                if not targets:
                    ...
                    for i, g in enumerate(targets, start=1):
                        filename = f"output_{_safe_name(split_field)}_{g}.csv"
                        csv_str  = out_df.loc[groups[g]].to_csv(index=False)
                        ok, msg  = upload_pipeline(
                            csv_bytes=csv_str.encode("utf-8"), filename=filename,
                            token=token_up, project_id=int(project_id_up), project_name=project_name_up,
                            session=session, connect_timeout=connect_timeout, read_timeout=read_timeout)
                        if ok: successes += 1
                        else: failures += 1; st.error(msg)
                        progress.progress(i/total)
                        if delay_secs: time.sleep(delay_secs)
                    if successes: st.success(f"Uploaded {successes}/{total} file(s).")
                    if failures == 0: st.toast("All uploads completed successfully.", icon="✅")
